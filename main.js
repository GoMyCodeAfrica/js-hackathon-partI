...
function substringBetweenMatches(str, subStr) {
	var start = str.indexOf(subStr) + subStr.length;
...
function both_ends(str) {
	var len = str.length;
	if (len < 2) return '';
	return str.slice(0, 2) + str.slice(len-2);
}
...
function fix_start(str) {
	var firstChar = str[0];
	var answer = firstChar;

	for (var i = 1; i < str.length; i++) {
		if (str[i] === firstChar) answer += "*";
		else answer += str[i];
	}
...
*/
function mix_up(a, b) {
	var aFront = a.slice(0, 2);
	var aBack = a.slice(2);
	var bFront = b.slice(0, 2);
	var bBack = b.slice(2);
	return bFront + aBack + aFront + bBack;
}
...
function verbing(str) {
	var len = str.length;
	if (len < 3) return str;
	if (str.slice(len-3) === 'ing') return str + 'ly';
	return str + 'ing';
}
...
function not_bad(str) {
	...
	if (notIndex !== -1 && badIndex !== -1 && notIndex < badIndex) {
		var replaceStr = str.slice(notIndex, badIndex+3);
		return str.replace(replaceStr, 'good');
	}
...
function front_back(str1, str2) {
	var str1MidIndex = Math.round(str1.length/2);
	var str2MidIndex = Math.round(str2.length/2);
	var aFront = str1.slice(0, str1MidIndex);
	var aBack = str1.slice(str1MidIndex);
	var bFront = str2.slice(0, str2MidIndex);
	var bBack = str2.slice(str2MidIndex);
